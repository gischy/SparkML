...
public class LogisticRegressionTrainAndTest1_2Sample100_500_1000 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int sampleSize : sampleSizes) {
			...
			List<LabeledPoint> labeledPointsTrueTest = new ArrayList<LabeledPoint>();

			for (int i = numberOfLinks; i < linksTrue.length; i++) {
				labeledPointsTrueTest.add(createLabeledPoint(linksTrue[i]));
			}
			...
			List<LabeledPoint> labeledPointsFalseTest = new ArrayList<LabeledPoint>();

			for (int i = numberOfLinks; i < linksFalse.length; i++) {
				labeledPointsFalseTest.add(createLabeledPoint(linksFalse[i]));
			}
			...
			log("anzahl links true (test): " + labeledPointsTrueTest.size());
			...
			pointListTest.addAll(labeledPointsFalseTest);
